...
class Authorization
{
  ...
  public function ValidateKeys( $data_keys)
  {
  	...
  	if ( !$access)
		{
          ...
          'message' => ConfigClass::get("messages.AUTH0003"),
        )
      ));
		}
    ...
    if ( !empty( $rows['data']))
    {
    	...
    	$date_expired = date("Y-m-d H:i:s", (strtotime(date($date_now)) + ConfigClass::get("config.expires_token")));
        ...
        'user_id' => $rows['data'][0]['user_id'],
			...
			$ret = PDOClass2::Insert( $data_acces_token);
    	...
    	else
    	{
            ...
            'http_code' => 'HTTP/1.1 403 Forbidden',
		        'message' => ConfigClass::get("messages.AUTH0004"),
		      )
		    ));
    ...
    else
    {
    	...
    	return ( MessagesClass::Response( array(
          ...
          'http_code' => 'HTTP/1.1 403 Forbidden',
	        'message' => ConfigClass::get("messages.AUTH0005"),
	      )
	    ));
  ...
  public function ValidateAccessToken( $data_keys)
  {
    ...
    $params['params'] = array( $data_keys['api_key'],	$data_keys['access_token']);
